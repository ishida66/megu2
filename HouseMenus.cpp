...
HouseMenus::HouseMenus(const int &x, const int &y, const  int &width, const  int &height, std::map<ID, item_library::house> *_library, SDL_Texture *newtexture, SDL_Renderer *renderer, const bool& mini)
{
	rect.x = x;
	rect.y = y;
	rect.w = width;
	...
	color.g = 0;
	color.r = 0;
	current_classification = "house";
	std::string xmlpath = "assets//xml//house_menu_config.xml";
	std::string xmltype = "house";
...
void HouseMenus::RenderButtons(SDL_Renderer *renderer)
{
	if (visible){
		if (marker_visible){
...
HouseMenus::~HouseMenus()
{
}
void HouseMenus::updateSlots(SDL_Renderer *renderer){
	//clean up first
	for (int i = 0; i < 3; ++i){
...
SDL_Texture* HouseMenus::loadTexture(std::string path, SDL_Renderer *renderer)
{
	//The final texture
