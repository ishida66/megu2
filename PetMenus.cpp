...
PetMenus::PetMenus(const int &x, const int &y, const  int &width, const  int &height, std::map<ID, item_library::megu> *_library, SDL_Texture *newtexture, SDL_Renderer *renderer, const bool &mini)
{
	rect.x = x;
	rect.y = y;
	rect.w = width;
	...
	color.g = 0;
	color.r = 0;
	current_classification = "megu";
	std::string xmlpath = "assets//xml//megutate_menu_config.xml";
	std::string xmltype = "megutate";
...
void PetMenus::RenderButtons(SDL_Renderer *renderer)
{
	if (visible){
		if (marker_visible){
...
void PetMenus::updateSlots(SDL_Renderer *renderer){
	//clean up first
	for (int i = 0; i < 3; ++i){
	...
	for (int i = 0; i < 3; ++i){
			unsigned int curr_index = current_page * 3 + i;
...
SDL_Texture* PetMenus::loadTexture(std::string path, SDL_Renderer *renderer)
{
	//The final texture
